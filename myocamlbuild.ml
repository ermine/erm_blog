(* OASIS_START *)
(* DO NOT EDIT (digest: 2670086c9fcf629fe56d4a729ce761b9) *)
...
open Ocamlbuild_plugin;;
let package_default =
  {MyOCamlbuildBase.lib_ocaml = [("erm_blog", [])]; lib_c = []; flags = []; }
  ;;
...
let () =
  dispatch
    (fun hook ->
       ...
       match hook with
         | Before_rules ->
             rule "copy sources for client"
               ~dep:"src/%.ml"
               ~prod:"client/%.ml"
               (fun env build ->
                  let src = env "src/%.ml"
                  and dst = env "client/%.ml" in
                    Cmd (S[P"cp"; A src; A dst])
               );
             
             rule "copy sources for server"
               ~dep:"src/%.ml"
               ~prod:"server/%.ml"
               (fun env build ->
                  let src = env "src/%.ml"
                  and dst = env "server/%.ml" in
                    Cmd (S[P"cp"; A src; A dst])
               );
             ...
             rule "cmxs"
               ...
               (fun env _build ->
                  ...
                  and cmxs = env "%.cmxs" in
                    Cmd (S[Px"ocamlopt"; A"-shared"; A"-linkall"; A cmxa;
                           A"-o"; A cmxs])
               );


             flag ["ocaml"; "infer_interface"; "use_eliom_type"] &
               S[A"-package"; A"ocsigen.ext.eliom.syntax.type"];
